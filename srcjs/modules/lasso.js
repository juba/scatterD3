import {lasso_fn} from "../plugins/lasso";
import * as d3 from "d3";

// Lasso functions to execute while lassoing
function start(lasso) {
    ...
    lasso.items()
        ...
        if (d3.select(this).classed('point-label-line')) {
            d.scatterD3_lasso_line_stroke = d.scatterD3_lasso_line_stroke ? d.scatterD3_lasso_line_stroke : d3.select(this).style("stroke");
            d.scatterD3_lasso_line_fill = d.scatterD3_lasso_line_fill ? d.scatterD3_lasso_line_fill : d3.select(this).style("fill");
            d.scatterD3_lasso_line_opacity = d.scatterD3_lasso_line_opacity ? d.scatterD3_lasso_line_opacity : d3.select(this).style("opacity");
        }
        })
...
function end(lasso, chart) {
    ...
    lasso.items()
        ...
        if (d3.select(this).classed('point-label-line')) { return d.scatterD3_lasso_line_fill; }
        ...
        if (d3.select(this).classed('point-label-line')) { return d.scatterD3_lasso_line_opacity; }
        ...
        if (d3.select(this).classed('point-label-line')) { return d.scatterD3_lasso_line_stroke; }
...
export function on(chart) {
    ...
    var lasso = lasso_fn()
    	...
    	.on("start", function() { start(lasso); })
	    .on("draw", function() { draw(lasso); })     
	    .on("end", function() { end(lasso, chart); }); 
    root.call(lasso);

    // Change cursor style
    root.style("cursor", "crosshair");
    // Change togglers state
    var menu_entry = d3.select("#scatterD3-menu-" + chart.settings().html_id + " .lasso-entry");
    var custom_entry = d3.select("#" + chart.settings().dom_id_lasso_toggle);
    if (!menu_entry.empty()) {
