...
export function setup_scales(chart) {
    ...
    if (settings.col_continuous) {
        if (settings.colors === null) {
            ...
            scales.color = d3.scaleSequential(d3[settings.colors]);
        }
        scales.color = scales.color
            .domain([d3.min(data, d => d.col_var),
                     d3.max(data, d => d.col_var)]);
    }
    // Ordinal color scale
    else {
        if (settings.colors === null) {
            ...
            scales.color = n <= 10 ? d3.scaleOrdinal(utils.custom_scheme10()) : d3.scaleOrdinal(d3.schemePaired);
            ...
            if (settings.colors.trim()[0] == "#") {
                ...
                scales.color = d3.scaleOrdinal(d3[settings.colors]);
            }
    ...
    if (settings.symbols === null) {
        scales.symbol = d3.scaleOrdinal().range(d3.symbols);
    } else if (Array.isArray(settings.symbols)) {
        scales.symbol = d3.scaleOrdinal().range(settings.symbols.map(d => symbol_table[d]));
    } else if (typeof (settings.symbols) === "string") {
        // Single string given
        scales.symbol = d3.scaleOrdinal().range(Array(symbol_table[settings.symbols]));
    } else if (typeof (settings.symbols) === "object") {
        scales.symbol = d3.scaleOrdinal()
    ...
    if (settings.sizes === null) {
        scales.size = d3.scaleLinear()
            ...
            .domain([d3.min(data, d => d.size_var),
            d3.max(data, d => d.size_var)]);
    } else if (typeof(settings.sizes) === "object") {
        scales.size = d3.scaleOrdinal()
    ...
    if (settings.opacities === null) {
        scales.opacity = d3.scaleLinear()
            .range([0.1, 1])
            .domain([d3.min(data, d => d.opacity_var),
            d3.max(data, d => d.opacity_var)]);
    } else if (typeof(settings.opacities) === "object") {
        scales.opacity = d3.scaleOrdinal()
