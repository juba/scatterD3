...
A leader line between the point and its label is automaticcaly drawn when the distance between both is above a certain threshold.


By using `labels_positions = "auto"`, labels positions can be computed to minimize overlapping :

```{r labels_auto}
scatterD3(data = mtcars, x = wt, y = mpg, lab = names, labels_size = 9 , labels_positions = "auto")
```
...
In the same way, you can specify custom symbol-value mapping by passing a vector of symbol names to the `symbols` argument. If the vector is named, then the symbols will be associated with their names within `symbol_var`. Available symbol names are : `"circle"`, `"cross"`, `"diamond"`, `"square"`, `"star"`, `"triangle"`, and `"wye"`.
...
And you can also specify custom opacity-value or size-value mapping by passing a named vector to `opacities` or `sizes` :
...
If your original R variable is a factor, its level orders should be preserved in the legend, both for symbols and colors.
...
When `col_var` is considered as continuous, you can specify a custom color palette
by giving the `colors` argument the name of a [d3-scale-chromatic interpolating 
function](https://github.com/d3/d3-scale-chromatic#sequential-single-hue). The default
is `"interpolateViridis"`. For example :
...
You can customize the tooltips placement with the `tooltip_position` argument. It can take as value a combination of `"top"` or `"bottom"` and `"left"` or `"right"` (the default is `"bottom right"`) :
...
For more specific use cases, you can represent some points as an arrow starting from the origin by using the `type_var` argument, and you can add a unit circle with `unit_circle = TRUE`.
...
Inside a shiny app, these arguments allow to zoom on a specific point programmatically with transitions. See the [sample scatterD3 shiny app](http://data.nozav.org/app/scatterD3/) for a demonstration.
