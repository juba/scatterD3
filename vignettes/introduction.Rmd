...
You can also use the `opacity_var` argument to specify each point opacity.
Values must be numbers between 0 and 1, and note that no legend for opacity is
drawn, though, as it is not a real variable-opacity mapping.

```{r opacity_var}
mtcars$opacity <- runif(nrow(mtcars))
scatterD3(data=mtcars, x=mpg, y=wt, opacity_var = opacity)
```
